...
import time
from Script_Training_Input_Pipeline import (  # Replace with your actual module name
...
def test_process_actions(spark, test_data):
    ...
    # Verify counts
    assert all_actions.count() == 7  # 3 clicks + 2 add_to_carts + 2 orders
...
def test_get_recent_actions(spark, test_data):
    ...
    assert recent_actions_default.count() == 0
...
def test_create_training_data(spark, test_data):
    ...
    assert len(cust1_data["action_items"]) == 1000  # 3 actions
    assert len(cust1_data["action_types"]) == 1000
...
@pytest.mark.skip("Run manually for performance testing") #remove skip to test it
def test_performance(spark, test_data):
    ...
    assert duration < 30.0  # Example threshold